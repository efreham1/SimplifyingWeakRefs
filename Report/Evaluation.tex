...
\begin{table}[t]
\centering
\caption[Optimisation variants]{Optimisation variants and their enabled optimisations. A tick indicates the optimisation is active, a dash indicates it is disabled. Throughout the analysis and discussion, variants are referred to by their canonical names as listed in this table (e.g.\ \texttt{clear\_path\_dyn}, \texttt{all}).}
...
Unlike the single-object benchmark, the strong references are not all released at once. Instead, the benchmark proceeds through five rounds of clearing. In each round, 20\,\% of the strong references are nulled in a randomised order, making the corresponding objects eligible for garbage collection. After each round, \texttt{System.gc()} is called explicitly to trigger a collection cycle. This gradual release pattern means that weak references and weak fields are discovered and processed across multiple \ac{GC} cycles rather than in a single burst. Furthermore, it tests how the pipeline performs when the referents have different sizes and lifetimes, which may affect locality and traversal cost.
...
GC Committed Memory (auxiliary \ac{GCr} memory) & kB & \ac{GCr}-related memory currently committed by the \ac{JVM}. Referred to as \emph{auxiliary \ac{GCr} memory}. \\
...
\begin{table}[t]
\centering
\caption[\ac{GCr} log metrics in the measurement pipeline]{\ac{GCr} log metrics used in the measurement pipeline. For the single-object benchmark, per-run maximum values are used, for the multi-object benchmark, per-run averages are used.}
...
Each \ac{GC} timing figure shows a composite plot: a violin plot (top) with the full per-run distribution and a median percentage-difference histogram (bottom) showing all variants relative to the \texttt{none} baseline, including \texttt{none} itself at 0\,\%. The single-object benchmark uses per-run maxima and the multi-object benchmark uses per-run averages, following the rationale in \cref{subsec:eval-method}. The median is used for comparison rather than the mean, as the non-symmetric distributions tend to skew the mean. Note that the violin plot y-axis does not start at zero, it is range-fitted to the observed values so that the shape of each distribution is easier to see. \ac{GC} timing results are shown for five metrics for each benchmark: Major Collection (\cref{fig:gc-single-major,fig:gc-multi-major}), Old Generation (\cref{fig:gc-single-old-gen,fig:gc-multi-old-gen}), Old Phase: Concurrent Mark (\cref{fig:gc-single-old-mark,fig:gc-multi-old-mark}), Old Phase: Concurrent Process Non-Strong (\cref{fig:gc-single-nonstrong,fig:gc-multi-nonstrong}), and Young Generation -- Promote All (\cref{fig:gc-single-young-gen,fig:gc-multi-young-gen}). Memory results are shown as median percentage-difference histograms of per-run maxima for three metrics: auxiliary \ac{GCr} memory (GC Committed Memory), Java heap, and total process \ac{RSS}, for both benchmarks (\cref{fig:mem-single-hist,fig:mem-multi-hist}). Figures for remaining \ac{GCr} metrics from \cref{tab:gc-log-metrics} are shown in \cref{app:additional-plots}.

\paragraph{How to read the figures.}
Each \ac{GC} timing figure contains two panels. The \textbf{top panel} (violin plot) shows the full empirical distribution of per-run values for each variant: the body width represents the density of observations at that value, the central horizontal line marks the median, and the whiskers reach to the 10th and 90th percentiles. Variants are ordered left to right: \texttt{none} (the unmodified baseline), the seven \texttt{WeakReference} pipeline variants, and \texttt{weak\_fields}. When two distributions overlap strongly (the body of one spans the body of the other) median differences between those variants should be treated as directionally indicative. The \textbf{bottom panel} (percentage-difference histogram) shows the median of each variant as a percentage change relative to the \texttt{none} baseline. Negative bars indicate lower (better) values. The percentage shown is (median$_{\mathrm{variant}}$ $-$ median$_{\mathrm{none}}$) / median$_{\mathrm{none}}$. The two panels together let the reader assess both the size of the effect (bottom panel) and the certainty of the comparison (top panel): a large bar paired with non-overlapping distributions indicates a clear effect, whilst a small bar paired with heavily overlapping distributions warrants caution in interpretation.

In addition to the figures, \Cref{tab:stats-ratio-slurm-5118807_combined_gc} shows the mean, median, and IQR of the fraction of major-collection time attributable to non-strong processing for the \texttt{none} baseline and the \texttt{all} variant, across both benchmarks. The fraction is computed by dividing the per-run values of the non-strong processing times by the major-collection times. Just as for the timing figures, the single-object benchmark uses per-run maxima and the multi-object benchmark uses per-run averages. In \Cref{fig:app-ratio-nonstrong-multi,fig:app-ratio-nonstrong-single} in \cref{app:additional-plots}, the full distributions of these ratios are shown as violin plots for both benchmarks.

\input{../tables/stats_ratio_slurm-5118807_combined_gc.tex}
...
\begin{figure}[t]
	...
	\caption[Major Collection timing (single-object)]{Major Collection timing, \textbf{single-object benchmark} (per-run maxima). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Major Collection timing (multi-object)]{Major Collection timing, \textbf{multi-object benchmark} (per-run averages). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Old Generation timing (single-object)]{Old Generation timing, \textbf{single-object benchmark} (per-run maxima). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Old Generation timing (multi-object)]{Old Generation timing, \textbf{multi-object benchmark} (per-run averages). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Old Phase: Concurrent Mark timing (single-object)]{Old Phase: Concurrent Mark timing, \textbf{single-object benchmark} (per-run maxima). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Old Phase: Concurrent Mark timing (multi-object)]{Old Phase: Concurrent Mark timing, \textbf{multi-object benchmark} (per-run averages). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Old Phase: Concurrent Process Non-Strong timing (single-object)]{Old Phase: Concurrent Process Non-Strong timing, \textbf{single-object benchmark} (per-run maxima). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Old Phase: Concurrent Process Non-Strong timing (multi-object)]{Old Phase: Concurrent Process Non-Strong timing, \textbf{multi-object benchmark} (per-run averages). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Young Generation (Promote All) timing (single-object)]{Young Generation (Promote All) timing, \textbf{single-object benchmark} (per-run maxima). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Young Generation (Promote All) timing (multi-object)]{Young Generation (Promote All) timing, \textbf{multi-object benchmark} (per-run averages). Lower is better. Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Memory median percentage differences (single-object)]{Median percentage difference in per-run maximum memory usage relative to the \texttt{none} baseline, \textbf{single-object benchmark}. Lower is better. Top to bottom: auxiliary \ac{GCr} memory, Java heap, total process \ac{RSS}.}
...
\begin{figure}[t]
	...
	\caption[Memory median percentage differences (multi-object)]{Median percentage difference in per-run maximum memory usage relative to the \texttt{none} baseline, \textbf{multi-object benchmark}. Lower is better. Top to bottom: auxiliary \ac{GCr} memory, Java heap, total process \ac{RSS}.}
