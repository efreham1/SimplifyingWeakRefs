...
\Cref{tab:nonstrong-fraction} summarises what fraction of median major-collection time is attributable to non-strong processing in the \texttt{none} baseline and in the best-performing pipeline variant (\texttt{all}). This fraction represents the ceiling on how much the pipeline optimisations can reduce total collection time: even a complete elimination of non-strong processing could not improve major-collection time by more than the listed percentages. In the single-object benchmark the fraction is 14.3\,\% for the baseline and falls to 2.9\,\% for \texttt{all}; in the multi-object benchmark the corresponding fractions are 4.5\,\% and 1.9\,\%. The ceiling is much tighter in the multi-object benchmark, reflecting the more diverse workload in which GC effort is spread across marking, relocation, and other phases. This context is important for interpreting the timing results in \cref{chap:analysis}: reductions in non-strong processing time do not translate directly into proportional improvements in major-collection time.
