...
\abstractdummy{
	...
	The results show that the strongest \texttt{WeakReference} variants are those that combine the specialised clear path with the dynamic-array representation. In the single-object stress benchmark, these variants reduce the targeted reference-processing subphases by more than 70\% and reduce major-collection time by up to 2.9\%. In the multi-object benchmark, subphase improvements remain substantial, but end-to-end major-collection gains are small. The exploratory weak-field mechanism performs strongly in the single-object benchmark but regresses in the multi-object benchmark, indicating that field-based weak semantics are promising but not yet a general replacement for optimised \texttt{WeakReference} processing.
    \end{abstract}
}
