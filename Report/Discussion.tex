...
The skip-enqueue optimisation was motivated by the observation that the enqueueing stage performed unnecessary work for queue-less weak references: the references were still linked into the pending list, transferred to the \texttt{ReferenceHandler} thread, and iterated over only to be discarded~\cite{openjdk-reference-java,openjdk-zreferenceprocessor-cpp,JDK-8029205}. Despite this, \texttt{sep\_only} reduces the single-object median by only 5\,\% and shows no reduction in the multi-object benchmark. When combined with the dynamic array in \texttt{sep\_dyn}, the improvement over \texttt{dyn\_only} is only 6 percentage points in the single-object benchmark and 2 percentage points in the multi-object benchmark. The same pattern continues in the \texttt{all} variant, which shows only a 0.3 percentage point increase over \texttt{clear\_path\_dyn} in the single-object benchmark and a 0.2 percentage point increase in the multi-object benchmark. Where \texttt{all} offers a meaningful advantage over \texttt{clear\_path\_dyn} is in auxiliary \ac{GC} memory: by routing queue-less references to the separate list, the array entries in \texttt{all} do not need to store the reference address field that \texttt{clear\_path\_dyn} requires, reducing auxiliary memory by approximately 30\,\% in the single-object benchmark (from 1\,268\,MB to 884\,MB).
