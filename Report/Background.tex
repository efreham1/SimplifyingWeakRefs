...
In addition to ordinary (strong) references, Java provides \emph{reference objects} in the \texttt{java.lang.ref} package~\cite{oracle-ref-javadoc} that allow applications to reference objects without preventing their garbage collection. These reference types are:
...
A reference object transitions through the following states during its lifetime~\cite{oracle-reference-javadoc}:
...
% TODO: citation needed -- no authoritative source quantifying WeakReference as the most
% commonly used non-strong reference type; claim is based on inspection of popular frameworks.
A \texttt{WeakReference} is the most commonly used non-strong reference type in Java applications. It has two constructors, as shown in \cref{lst:weakref-constructors}.
...
\begin{enumerate}
    ...
    \item \textbf{Generational considerations.} In ZGC, only references residing in the old generation are discovered; young-generation references are always treated as strong. This is because reference processing introduces additional delay and uncertainty to the duration of the young-generation collection. By treating young references as strong, ZGC can complete young collections more predictably, which decreases the risk of the application running out of memory during a long young collection. 
\end{enumerate}
