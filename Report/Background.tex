...
\begin{itemize}
    ...
    \item \texttt{discovered field}   A field with dual purpose: during \ac{GC} marking, it serves as a link in the \ac{GCr}'s internal \emph{discovered list}, after processing, it is reused as a link in the VM's \emph{pending list} that feeds the \texttt{ReferenceHandler} thread. Due to its first use, the \ac{GCr} can detect whether a reference has already been discovered by checking if this field is \texttt{null}.
\end{itemize}
