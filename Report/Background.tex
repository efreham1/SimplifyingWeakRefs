...
To answer the research questions posed in \cref{sec:research-questions}, it is necessary to understand key concepts of the Java Virtual Machine's (JVM) garbage collection mechanisms, the design and implementation of the Z Garbage Collector (ZGC), the Java reference types and their processing pipeline within ZGC, and general principles of garbage collection. This chapter provides an overview of these topics, which will serve as the foundation for the implementation and evaluation of the optimisations explored in this thesis.
...
\label{subsec:gc-barriers-general}
...
\label{sec:jvm-execution}

To understand how garbage-collector barriers are injected and why ZGC requires different components for each execution tier, we must examine HotSpot's tiered execution model, which balances startup latency against sustained throughput. Three tiers are relevant to this thesis:

\begin{enumerate}
    \item \textbf{Interpreter.} At JVM startup, an \emph{interpreter} is generated in which each bytecode is mapped to a hand-crafted assembly stub~TODO. Methods not yet compiled are executed in this tier. Interpretation is the slowest tier in terms of throughput but has no compilation overhead, and it also gathers profiling data for JIT compilation.

    \item \textbf{C1 Compiler.} C1 is a fast JIT compiler that performs lightweight optimisations, such as method inlining, constant folding, and null-check elimination~TODO. C1 compiles quickly, making it suitable for methods that are called moderately often.

    \item \textbf{C2 Compiler.} C2 is an aggressive optimising JIT compiler. It consumes profiling data collected by the interpreter and C1 to emit highly optimised native code, applying techniques such as aggressive inlining, loop transformations, and escape analysis~TODO. Hot methods are ultimately compiled by C2.
\end{enumerate}

\emph{Tiered compilation}, the default since TODO, automatically promotes methods through these tiers as their execution frequency increases. A method starts in the interpreter, is promoted to C1-compiled code once it becomes warm, and may eventually be recompiled by C2 once sufficient profiling data has accumulated.

\paragraph{Relevance to GC barriers.} Because a read barrier must intercept every load from the heap, and such loads occur in all three tiers, each tier requires its own barrier implementation. ZGC therefore ships three separate barrier components: \texttt{ZBarrierSetAssembler} generates barrier stubs embedded in the interpreter's dispatch table, \texttt{ZBarrierSetC1} inserts barrier nodes during C1 intermediate representation construction, and \texttt{ZBarrierSetC2} integrates barriers into the C2 sea-of-nodes graph~\cite{openjdk-zbarrier-cpp}. All three components obey the same decorator-driven barrier selection logic described in \cref{subsec:access-decorators}.
...
\label{subsec:coloured-pointers-and-barriers}
...
\label{subsec:concurrent-phases-and-workers}
...
\label{subsec:access-decorators}
...
\begin{itemize}
    ...
    \item \textbf{Access location} bits indicates whether the access targets a heap object field or an off-heap native data structure. Some barrier actions are conditioned on \texttt{IN\_HEAP}.
