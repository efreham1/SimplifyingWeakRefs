...
bool discover_reference(oop ref_obj, ReferenceType type) {
    zaddress ref = to_zaddress(ref_obj); // convert to ZGC-coloured pointer

    if (!should_discover(ref, type)) {   // skip already-discovered or ineligible refs
        return false;
    }

    if (type == REF_WEAK && !has_reference_queue(ref)) {
        // Queue-less weak ref: goes to the no-enqueue list, bypasses
        // pending-list hand-off and ReferenceHandler thread entirely
        weak_no_queue_list_per_worker.append(ref);
    } else {
        // All other reference types (and queue-backed weak refs) follow
...
void discover_reference(oop ref_obj, ReferenceType type) {
    //...
    ZWeakRefData data;
    data.set_from_ref(ref);       // pre-load referent field address and value
    worker_array.append(data);    // O(1) amortised; array grows on the C heap if needed
    //...
}
...
void process_worker_discovered_weak_refs_without_queue(ZWeakRefArray& arr) {
    ...
    for (size_t i = 0; i < arr.length(); i++) {
        const ZWeakRefData& data = arr.at(i); // index-based: contiguous, cache-friendly
        data.mark_not_discovered();            // clear discovered field before liveness check

        if (try_make_inactive(data)) { // clear referent if no longer strongly reachable
            cleared_weak_no_queue_count++;
        } else {
            dropped++; // reference stayed active; slot kept for next GC cycle
        }
    }
    arr.clear_and_reserve(dropped); // retain capacity to avoid reallocation next cycle
}
\end{lstlisting}
...
To answer \cref{rq:weak-fields}, an exploratory weak-field mechanism was implemented to test whether expressing weak semantics directly on object fields can reduce the overhead of \texttt{WeakReference} objects. The mechanism allows developers to annotate object fields as weak, indicating that the \ac{GC} should treat those fields with weak semantics. The \ac{GC} then applies weak semantic rules directly to the annotated field without requiring a separate \texttt{WeakReference} object. This is expected to slightly reduce reference-processing overhead and to reduce Java heap usage by eliminating the need for separate \texttt{WeakReference} objects. The reduction in Java heap usage should also reduce \ac{GC} cycle time, since garbage-collection cost is broadly proportional to the number of live objects~\cite{oracle-gc-implementation}.

\subsection{Implementation of Weak Fields}
\label{subsec:di-wf-impl}

The implementation includes changes in three layers of the \ac{JVM}: a Java-level annotation, \ac{GC}-side field discovery and processing, and compiler/interpreter support for the annotation metadata.
...
\end{lstlisting}
\caption[Weak-field annotation]{The \texttt{@weak} annotation. Applied to a field, it instructs the \ac{GC} to treat the field value with weak-reference semantics (see \cref{app:source-wf-annotation}).}
...
bool discover_weak_field(oop* field) {
    ...
    // Record field address and current pointer value for the processing phase
    discovered_weak_fields_count++;
    ZWeakFieldData data(field);
    worker_weak_field_array.append(data); // per-worker array avoids contention
    return true;
}
\end{lstlisting}
...
void process_worker_discovered_weak_fields(ZWeakFieldArray& weak_fields) {
    ...
    for (size_t i = 0; i < weak_fields.length(); i++) {
        const ZWeakFieldData& data = weak_fields.at(i); // sequential: cache-friendly
        if (try_make_inactive_fast(data)) { // clear dead referent or recolour live one
            cleared_weak_fields_count++;
        } else {
            dropped++; // field stayed active; reserve space for next GC cycle
        }
    }
    weak_fields.clear_and_reserve(dropped); // avoid reallocation if count is stable
}
\end{lstlisting}
...
The \texttt{@weak} annotation metadata is also made available to the \ac{JIT} compilers and interpreter. This is necessary to ensure that any loads or stores to a weak field outside \ac{GC} processing are handled correctly with respect to weak semantics. The implementation adds a decorator to loads and stores that target a field carrying the \texttt{weak} flag in its metadata. \ac{ZGC} then inserts barriers that apply the required semantics to all accesses with this decorator. Without the load decorator, a load from a weak field could return a non-null reference whose referent will be cleared by the \ac{GC}. The store decorator is needed so that the \ac{GC} does not record the previous value of the field, unlike in the default strong-reference case used to preserve the \ac{SATB} marking semantics. See \cref{subsec:bg-zgc-decorators} for more details on decorators and \cref{app:source-wf-compiler} for the implementations.
