\chapter{Usage of GenAI Tools}
\label{app:ai-usage}

The \ac{GenAI} tool GitHub Copilot was used as support during this thesis. It was used under a student licence via Uppsala University. The tool was used to accelerate practical work and improve writing quality. All final technical decisions, implementation choices, and evaluations remained under the author's control.
...
\begin{figure}[t]
	...
	\caption[Old Phase: Concurrent Relocate timing (single-object)]{Old Phase: Concurrent Relocate timing, \textbf{single-object benchmark} (per-run maxima). Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Old Phase: Concurrent Relocate timing (multi-object)]{Old Phase: Concurrent Relocate timing, \textbf{multi-object benchmark} (per-run averages). Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Young Phase: Concurrent Mark timing (single-object)]{Young Phase: Concurrent Mark timing, \textbf{single-object benchmark} (per-run maxima). Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Young Phase: Concurrent Mark timing (multi-object)]{Young Phase: Concurrent Mark timing, \textbf{multi-object benchmark} (per-run averages). Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Young Phase: Concurrent Relocate timing (single-object)]{Young Phase: Concurrent Relocate timing, \textbf{single-object benchmark} (per-run maxima). Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Young Phase: Concurrent Relocate timing (multi-object)]{Young Phase: Concurrent Relocate timing, \textbf{multi-object benchmark} (per-run averages). Top: distribution, bottom: median \,\% difference vs baseline.}
...
\begin{figure}[t]
	...
	\caption[Memory per-run maximum distributions (single-object)]{Distribution of per-run maximum memory across variants, \textbf{single-object benchmark}. Top to bottom: auxiliary \ac{GCr} memory, Java heap, total process \ac{RSS}.}
...
\begin{figure}[t]
	...
	\caption[Memory per-run maximum distributions (multi-object)]{Distribution of per-run maximum memory across variants, \textbf{multi-object benchmark}. Top to bottom: auxiliary \ac{GCr} memory, Java heap, total process \ac{RSS}.}
	\label{fig:app-mem-multi-max}
\end{figure}

% ---------- Ratio of non-strong processing time to total major collection time – single ----------
\begin{figure}[t]
	\centering
	\includegraphics[width=\textwidth]{ratio_violin_slurm-5118807_combined_gc_multi.pdf}
	\caption[Ratio of non-strong processing time to total major collection time]{Ratio of non-strong processing time to total major collection time, \textbf{multi-object benchmark} (per-run averages). Distribution across variants.}
	\label{fig:app-ratio-nonstrong-multi}
\end{figure}

% ---------- Ratio of non-strong processing time to total major collection time – multi ----------
\begin{figure}[t]
	\centering
	\includegraphics[width=\textwidth]{ratio_violin_slurm-5118807_combined_gc_single.pdf}
	\caption[Ratio of non-strong processing time to total major collection time]{Ratio of non-strong processing time to total major collection time, \textbf{single-object benchmark} (per-run maxima). Distribution across variants.}
...
The \texttt{ZWeakRefArray} template is the central data structure for the dynamic-array optimisation. It stores discovered weak references contiguously on the C heap, enabling sequential traversal without pointer chasing. Each entry in the extended variant (\texttt{ZWeakRefDataExt}) pre-loads the referent field address and value during discovery so that the clear path can operate without additional heap loads. Note in particular the \texttt{clear\_and\_reserve} method, which retains capacity between \ac{GC} cycles to avoid repeated reallocation when the reference population is stable.
...
The header file shows the per-worker state added by the \texttt{all} variant: a \texttt{ZWeakRefArray} for queue-less discovered references (bypassing the pending list), a separate \texttt{ZWeakRefArray} used by the standard pipeline, and a counter for cleared weak references without a queue. The key type change to notice is \texttt{ZWeakRefDataExt} in the queue-less path -- this is the extended entry type that pre-loads the referent for the barrier-free clear path.
...
The \texttt{@weak} annotation is the user-facing entry point for the weak-field mechanism. It is intentionally minimal -- a marker annotation with \texttt{@Target(ElementType.FIELD)} and \texttt{@Retention(RetentionPolicy.RUNTIME)}. The runtime retention is required because the class-file parser must be able to detect the annotation on field descriptors when loading the class; it is not consumed by the JIT compilers, which instead query the resolved field entry.
...
The \texttt{ZWeakFieldArray} is the weak-fields counterpart of \texttt{ZWeakRefArray}. The key structural difference is that each entry records the \emph{field address} (a pointer into the containing object's fields) rather than the address of a \texttt{WeakReference} wrapper object. This is the detail that allows the \ac{GC} worker to clear the field in-place without involving the reference pipeline at all. The entry size is smaller than \texttt{ZWeakRefDataExt} because there is no queue or discovered field to track.
