...
void ZReferenceProcessor::process_worker_discovered_array(ZWeakRefArray& all_refs) {
  ...
  for (size_t i = 0; i < all_refs.length(); i++) {
    ...
    const ReferenceType type = data.type;

    if (type == REF_WEAK && data.has_reference_queue == false) {
