...
The agenda for my presentation looks like this. We'll go through the background, the motivation, the design and implementation, the evaluation method, the results, and finally the conclusions. And please hold any questions until the end.
...
ZGC is a concurrent relocating garbage collector. So what does that mean? It means that it runs concurrently with the application - resulting in very short application pauses or stop-the-world pauses as they're more commonly referred to. It also means that it relocates alive objects to new memory and then lets that old memory be reused.
...
Once this is done, we get to the relocating phase, which is basically that we take C and move it to a new piece of memory over here. And when that is done, we mark the old memory region as reusable, meaning it can eventually be overwritten by new objects. So that's your crash course in how garbage collection works.
